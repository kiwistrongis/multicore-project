...
const BROKERS_N : u32 = 100;
const EXCHANGE_DAYS : u32 = 10000;
const EXCHANGE_DELAY : u32 = 20;
...
fn main(){
	...
	for i in 0..BROKERS_N {
		...
		{ // temporary scope due to `stock` borrow
			...
			arg.buy_price = stock_inner.price - 1*( 1 + x);
			arg.sell_price = stock_inner.price + 1*( 1 + x);
			drop( stock_inner);}
